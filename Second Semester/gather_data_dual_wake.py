...
def get_avg(lead,mid,third,angle_lower,angle_higher,speed):
    ...
    comparison_column_1 = np.where(final_df[lead+'_Grd_Prod_Pwr_Avg'] > final_df[mid+'_Grd_Prod_Pwr_Avg'], True, False)
    comparison_column_2 = np.where(final_df[lead+'_Grd_Prod_Pwr_Avg'] > final_df[mid+'_Grd_Prod_Pwr_Avg'], True, False)
    comparison_column_3 = np.where(final_df[lead+'_Grd_Prod_Pwr_Avg'] > final_df[third+'_Grd_Prod_Pwr_Avg'], True, False)
    comparison_column_4 = np.where(final_df[mid+'_Grd_Prod_Pwr_Avg'] > final_df[third+'_Grd_Prod_Pwr_Avg'], True, False)

    final_df['comparison_1'] = comparison_column_1
    final_df['comparison_2'] = comparison_column_2
    final_df['comparison_3'] = comparison_column_3
    final_df['comparison_4'] = comparison_column_4
    ...
    (final_df[third+'_Grd_Prod_Pwr_Avg'] > 0) &
    (final_df['comparison_1'] == True) &
    (final_df['comparison_2'] == True) &
    (final_df['comparison_3'] == True) &
    (final_df['comparison_4'] == True) &
    (final_df[lead+'_Grd_Prod_Pwr_InternalDerateStat']<4) & #this removes curtailed values
    ...
    return [np.asarray(power_df[lead+'_Grd_Prod_Pwr_Avg']),np.asarray(power_df[mid+'_Grd_Prod_Pwr_Avg']),np.asarray(power_df[third+'_Grd_Prod_Pwr_Avg'])]
...
def getavg():
    ...
    lead_power_array  = [lead_power_array for lead_power_array in lead_power_array if str(lead_power_array) != 'nan']
    mid_power_array  = [mid_power_array for mid_power_array in mid_power_array if str(mid_power_array) != 'nan']
    third_power_array  = [third_power_array for third_power_array in third_power_array if str(third_power_array) != 'nan']

    lead_df = pd.DataFrame({'lead':lead_power_array})
    mid_df = pd.DataFrame({'mid':mid_power_array})
    rear_df = pd.DataFrame({'rear':third_power_array})
    path = './saved_power_data'
    os.makedirs(path,exist_ok=True)

    lead_df.to_csv(path+'/lead.csv')
    mid_df.to_csv(path+'/mid.csv')
    rear_df.to_csv(path+'/rear.csv')

    # path = '/saved_mean_power'
    # os.makedirs(path,exist_ok=True)
    # np.savetxt(path+"/savedPowerAverage.csv", file, delimiter=",")
