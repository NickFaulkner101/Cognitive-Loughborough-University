...
#------------------------total power of farm-------------------------
    generation = sum(power_kw)/1000 #total power of farm at the windspeed, in MW
    total_free_power_mw = num_turbines*power_freestream/1000

    efficiency = (generation/total_free_power_mw)

    loss = (1-efficiency)*100


    return [round(generation,2), round(total_free_power_mw,2), round(loss,2)]
...
#--------------this grid is used to calculate wind speeds for multiple turbines
    ...
    #wind farm farm_stats
 

    farm_stats_data = farm_stats(kw_power_list,V0)


    print(farm_stats_data)




    print('-------------------Wind Farm Simulated!------------------------\n\nGeneration: '+str(farm_stats_data[0])+' MW')
    print('\nPotential Generation: '+str(farm_stats_data[1])+' MW')
    print('\nLoss: '+str(farm_stats_data[2])+'%')
    
    plt.figure(1, figsize=(13,5))
    plt.suptitle('Wind Farm Yield: '+str(farm_stats_data[0])+' MW. Loss= '+str(farm_stats_data[2])+'%'+' Windspeed: '+str(V0)+' m/s'+' Wind Direction: '+str(angle)+' °')
    ...
    plt.subplot(1, 2, 2)
    plt.xlabel('Turbine')
    plt.title('Generation Per Turbine')
    plt.ylabel('Yield /MW')
    plt.grid(color='black', linestyle='-', linewidth=0.25)
    plt.bar( [str(int) for int in turbine_no],np.array(kw_power_list)/1000, color='green')
