...
def start():
    ...
    range_of_speeds = np.linspace(3,15,13)
    ...
    angles_through_west=np.arange(0,num)*step+start
    angles_through_west = angles_through_west*-1

    for i in range(0,len(angles_through_west)):
        for v in range(0,len(range_of_speeds)):
            main(angles_through_west[i],range_of_speeds[v])
